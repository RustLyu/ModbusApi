...
void printErrorMsg(Modbus* context)
{
	switch (context->getErrorFlag())
	{
	case ERRORFLAG_NONE:
	{
...
bool isConnect(Modbus* ctx)
{
	if (ctx != nullptr)
	{
		return ctx->isConnect();
	}
	return false;
}

bool setMaxRequestTimeOut(Modbus* ctx, uint32_t sec, uint32_t umesc)
{
	if (ctx == nullptr)
	{
		return false;
	}
	ctx->setRequestTimeOut(sec, umesc);
	return true;
}

bool setMaxResponseTimeOut(Modbus* ctx, uint32_t sec, uint32_t umesc)
{
	if (ctx == nullptr)
	{
		return false;
	}
	ctx->setResponseTimeOut(sec, umesc);
	return true;
}

int readIOStatus(Modbus* context, int func, int addr, int count, uint16_t* dst)
{
	if (context == nullptr)
	{
	...
	if (ret > 0)
	{
		...
		int rc = check_confirmation(context, _MODBUS_TCP_HEADER_LENGTH, req, rsp, len);
		if (rc < 0)
		{
			return -1;
		}
...
int readRegisters(Modbus* context, int func, int addr, int count, uint16_t* dst)
{
	if (context == nullptr)
	{
		return MODBUS_RET_ERROR;
...
int readMulBit(Modbus* context, int addr, int count, uint16_t* dst)
{
	return readIOStatus(context, MODBUS_FC_READ_COILS, addr, count, dst);
}

int readInputMulBit(Modbus* context, int addr, int count, uint16_t* dst)
{
	return readIOStatus(context, MODBUS_FC_READ_DISCRETE_INPUTS, addr, count, dst);
}

int readHoldMulRegister(Modbus* context, int addr, int count, uint16_t* dst)
{
	return readRegisters(context, MODBUS_FC_READ_HOLDING_REGISTERS, addr, count, dst);
}

int readInputMulRegister(Modbus* context, int addr, int count, uint16_t* dst)
{
...
int writeMulBit(Modbus* context, int addr, int count, uint16_t* src)
{
	if (context == nullptr)
	{
		return MODBUS_RET_ERROR;
	...
	else
	{
		...
		check_confirmation(context, _MODBUS_TCP_HEADER_LENGTH, req, rsp, len);
	}
	return MODBUS_RET_OK;
}

int writeMulRegister(Modbus* context, int addr, int count, uint16_t* src)
{
	if (context == NULL)
	{
		return MODBUS_RET_ERROR;
